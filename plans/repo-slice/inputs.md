...
- `src/repo/types.ts` — current repo types (zod schemas: `RepoEntry` with optional `id`, `RepoRegistry`)
...
- `src/repo/list.test.ts` — existing list logic tests
- `src/repo/state.test.ts` — existing state management tests
